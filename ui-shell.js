(function () {
  ...
  function menuItem(action, label, key = "", iconName = "caret-right", id = "", attributes = "") {
    return `<button role="menuitem" ${id ? `id="${id}"` : ""} ${action ? `data-action="${action}"` : ""} ${attributes}>${icon(iconName, "sm")}<span>${label}</span>${key ? `<kbd>${key}</kbd>` : ""}</button>`;
  }

  function menusMarkup() {
    return `<div class="menu-popover" id="menuFile" role="menu" hidden>${menuItem("new-map", "New map", "", "plus")}${menuItem("", "Import map", "", "upload-simple", "importBtn")}${menuItem("", "Export map", "", "download-simple", "exportBtn")}${menuItem("", "Export as image", "", "image", "exportImageBtn")}<hr>${menuItem("", "Export saved shapes", "", "download-simple", "exportCustomShapesBtn")}${menuItem("", "Import saved shapes", "", "upload-simple", "importCustomShapesBtn")}</div>
      <div class="menu-popover" id="menuEdit" role="menu" hidden>${menuItem("undo", "Undo", "Ctrl Z", "arrow-counter-clockwise")}${menuItem("redo", "Redo", "Ctrl Y", "arrow-clockwise")}<hr>${menuItem("cut", "Cut", "Ctrl X", "scissors")}${menuItem("copy", "Copy", "Ctrl C", "stack-simple")}${menuItem("paste", "Paste", "Ctrl V", "plus")}${menuItem("delete", "Delete", "Del", "trash")}<hr>${menuItem("select-matching", "Select matching objects")}${menuItem("", "Move map to origin", "", "crosshair-simple", "centerMapOriginBtn")}${menuItem("", "Make all towers invincible", "", "shield-check", "makeAllTowersInvincibleBtn")}</div>
      <div class="menu-popover" id="menuView" role="menu" hidden>${menuItem("fit", "Zoom to fit map", "F", "magnifying-glass")}<div class="menu-heading">Grid</div>${menuItem("grid-visible", "Show grid", "G", "grid-four")}${menuItem("grid-major", "Emphasise every 5th line", "", "check-circle")}${menuItem("origin-axes", "Show origin axes", "", "check-circle")}<hr><div class="menu-heading">Snapping</div>${menuItem("grid-snap", "Snap to grid", "", "check-circle")}${menuItem("object-snap", "Snap to objects", "", "check-circle")}${menuItem("move-snap", "Snap while moving", "", "check-circle")}${menuItem("build-snap", "Snap while building", "", "check-circle")}</div>`;
  }

  function settingsRow(title, description, control, attributes = "") {
    return `<div class="settings-row" ${attributes}><div><strong>${title}</strong><span>${description}</span></div><div>${control}</div></div>`;
  }
  ...
  function settingsMarkup() {
    return `<div id="settingsPanel" class="modal-scrim hidden"><section class="settings-modal" role="dialog" aria-modal="true" aria-label="Settings"><header>${icon("gear-six")}<h2>Settings</h2><button id="settingsCloseBtn" class="icon-btn" aria-label="Close settings">${icon("x")}</button></header><div class="settings-layout"><nav role="tablist"><button class="settings-tab active" data-settings-tab="canvas">${icon("grid-four", "sm")}Canvas and grid</button><button class="settings-tab" data-settings-tab="snapping">${icon("magnet", "sm")}Snapping</button><button class="settings-tab" data-settings-tab="rules">${icon("shield-check", "sm")}Map rules</button><button class="settings-tab" data-settings-tab="multiplayer">${icon("users-three", "sm")}Multiplayer</button><button class="settings-tab" data-settings-tab="data">${icon("floppy-disk", "sm")}Import and export</button></nav><div class="settings-content">
      <section data-settings-pane="canvas"><h3>Canvas and grid</h3><p>Local workspace appearance.</p>${settingsRow("Theme", "Dark or light interface and canvas.", '<select id="themeSelect"><option value="dark">Dark</option><option value="light">Light</option></select>')}${settingsRow("Show grid", "Draw the regular grid lines.", switchControl("gridVisibleInput", "", true))}${settingsRow("Grid size", "Spacing in map units.", '<input id="gridSizeInput" type="number" value="48" min="4" max="1000" step="1">')}${settingsRow("Grid line thickness", "Line width in pixels.", '<input id="gridLineWidthInput" type="number" value="1" min="0.25" max="8" step="0.25">')}${settingsRow("Emphasise every 5th line", "Draw major lines independently of the regular grid.", switchControl("gridMajorVisibleInput", "", true))}${settingsRow("Show origin axes", "Draw the 0,0 crosshair independently.", switchControl("originAxesVisibleInput", "", true))}</section>
      <section data-settings-pane="snapping" hidden><h3>Snapping</h3><p>Hold Ctrl to suspend snapping temporarily.</p>${settingsRow("Snap to grid", "Land on grid intersections.", switchControl("gridSnapEnabledInput", "", true))}${settingsRow("Snap to objects", "Align to nearby objects.", switchControl("objectSnapEnabledInput", "", true))}${settingsRow("Snap while moving", "Apply grid and object snapping when moving a selection.", switchControl("moveSnapEnabledInput", "", true))}${settingsRow("Snap while building", "Use object snapping in Build.", switchControl("buildSnapEnabledInput", "", true))}${settingsRow("Snap strength", "Pull radius in map units.", '<input id="snapStrengthInput" type="number" value="20" min="1" step="1">')}</section>
      ...
      <section data-settings-pane="data" hidden><h3>Import and export</h3><p>Map files and saved shapes move independently.</p>${settingsRow("Map files", "Import JSON or TXT; export the map or a PNG from File.", '<span class="readonly">File menu</span>')}${settingsRow("Foreign map conversion", "Starts automatically when a TXT file is imported.", '<span class="readonly">Automatic</span>')}${settingsRow("Saved shapes", "Import and export the library from the File menu.", '<span class="readonly">File menu</span>')}</section>
      </div></div></section></div>`;
  }

  function sessionMarkup() {
    return `<div class="menu-popover session-popover" id="sessionPopover" role="dialog" aria-label="Live session" hidden>
      <div class="session-popover-head"><div><strong>Live session</strong><span id="sessionStateLabel">Offline</span></div><span class="session-count" id="sessionEditorCount">1 local</span></div>
      <div id="sessionParticipantList" class="session-participants" aria-label="Session editors"></div>
      <div class="session-divider"></div>
      <label class="session-name"><span>Name</span><input id="sessionUsernameInput" type="text" maxlength="32" placeholder="Player"></label>
      <p id="sessionStatusText" class="session-status" data-tone="info">Not connected. Start a session to invite collaborators.</p>
      <button id="sessionInviteBtn" class="session-action" type="button">${icon("link-simple", "sm")}<span>Start session &amp; copy invite link</span></button>
      <button id="sessionLeaveBtn" class="session-action danger" type="button" hidden>${icon("x", "sm")}<span>Leave session</span></button>
    </div>`;
  }
  ...
  function buildNewShell() {
    shellNew.innerHTML = `<div class="app"><header class="topbar"><div class="brand-name">Cosmowar Editor</div><nav class="menubar" aria-label="Main menu"><button class="menu-btn" data-menu="menuFile" aria-haspopup="true" aria-expanded="false">File</button><button class="menu-btn" data-menu="menuEdit" aria-haspopup="true" aria-expanded="false">Edit</button><button class="menu-btn" data-menu="menuView" aria-haspopup="true" aria-expanded="false">View</button></nav><span class="top-separator"></span><button class="icon-btn" data-action="undo" title="Undo (Ctrl+Z)" aria-label="Undo">${icon("arrow-counter-clockwise")}</button><button class="icon-btn" data-action="redo" title="Redo (Ctrl+Y)" aria-label="Redo">${icon("arrow-clockwise")}</button><span class="top-spacer"></span><button class="session-chip is-offline" id="sessionMenuBtn" data-menu="sessionPopover" type="button" aria-haspopup="dialog" aria-expanded="false"><span class="live-dot"></span><span data-session-label>Session</span></button><button class="icon-btn" id="themeToggleBtn" title="Switch theme" aria-label="Switch theme">${icon("sun")}</button><button class="icon-btn" id="settingsToggleBtn" title="Settings (Ctrl+,)" aria-label="Open settings">${icon("gear-six")}</button><button class="icon-btn" id="shortcutsToggleBtn" title="Keyboard shortcuts (?)" aria-label="Keyboard shortcuts">${icon("keyboard")}</button></header>
      ...
      <div id="rightResizeHandle" class="inspector-resize-handle" role="separator" aria-label="Resize inspector" aria-orientation="vertical"></div><aside class="inspector right-sidebar" aria-label="Inspector"><header class="inspector-header">${icon("selection-plus")}<strong>Inspector</strong></header><div class="inspector-scroll">${conversionMarkup()}<div id="selectionPanel" class="selection-panel"></div></div></aside><div id="leftResizeHandle" hidden></div></div></div>${menusMarkup()}${sessionMarkup()}${settingsMarkup()}${shortcutsMarkup()}<div id="multiplayerToastStack" class="toasts multiplayer-toast-stack" aria-live="polite" aria-atomic="false"></div><input type="file" id="importFileInput" accept=".json,.txt,application/json,text/plain" hidden><input type="file" id="customShapesFileInput" accept=".json,application/json" hidden>`;
  }
  ...
  function openMenu(button) { const menu = document.getElementById(button.dataset.menu); closeMenus(); if (!menu) return; const rect = button.getBoundingClientRect(); menu.hidden = false; const left = Math.min(rect.left, window.innerWidth - menu.offsetWidth - 8); menu.style.left = `${Math.max(8, left)}px`; menu.style.top = `${rect.bottom + 4}px`; button.setAttribute("aria-expanded", "true"); activeMenu = menu.id; }
  ...
  const persistentViewActions = new Set(["grid-visible", "grid-major", "origin-axes", "grid-snap", "object-snap", "move-snap", "build-snap"]);
  function runAction(action) { const api = globalThis.CosmowarEditorUI; const click = (id) => document.getElementById(id)?.click(); const actions = { "new-map": () => api?.newMap?.(), import: () => click("importBtn"), export: () => click("exportBtn"), "export-shapes": () => click("exportCustomShapesBtn"), "import-shapes": () => click("importCustomShapesBtn"), undo: () => api?.undo?.(), redo: () => api?.redo?.(), cut: () => api?.cut?.(), copy: () => api?.copy?.(), paste: () => api?.paste?.(), delete: () => api?.delete?.(), "select-matching": () => api?.selectMatching?.(), origin: () => click("centerMapOriginBtn"), invincible: () => click("makeAllTowersInvincibleBtn"), fit: () => api?.fit?.(), "grid-visible": () => toggleCheckbox("gridVisibleInput"), "grid-major": () => toggleCheckbox("gridMajorVisibleInput"), "origin-axes": () => toggleCheckbox("originAxesVisibleInput"), "grid-snap": () => toggleCheckbox("gridSnapEnabledInput"), "object-snap": () => toggleCheckbox("objectSnapEnabledInput"), "move-snap": () => toggleCheckbox("moveSnapEnabledInput"), "build-snap": () => toggleCheckbox("buildSnapEnabledInput") }; actions[action]?.(); if (!(activeMenu === "menuView" && persistentViewActions.has(action))) closeMenus(); }
  ...
  function setupMirrors() {
    legacyIds.forEach((legacy, id) => {
      ...
      if (!("value" in canonical) && canonical.tagName !== "BUTTON") {
        new MutationObserver(() => syncMirroredControl(canonical, legacy)).observe(canonical, { childList: true, subtree: true, characterData: true, attributes: true });
      }
    });
  ...
  shellNew.querySelectorAll(".menu-popover button").forEach((button) => button.addEventListener("click", () => { if (button.closest("#menuView") && persistentViewActions.has(button.dataset.action)) return; setTimeout(closeMenus); }));
  ...
  function setLibraryOpen(open) { const library = document.getElementById("shapeLibrary"); const button = document.getElementById("shapeLibraryToggleBtn"); library.hidden = !open; newHost.classList.toggle("lib-open", open); button.classList.toggle("active", open); button.setAttribute("aria-pressed", String(open)); }
  document.getElementById("shapeLibraryToggleBtn").addEventListener("click", () => setLibraryOpen(document.getElementById("shapeLibrary").hidden)); document.getElementById("shapeLibraryCloseBtn").addEventListener("click", () => setLibraryOpen(false));
  shellNew.querySelectorAll(".tool-button[data-tool]").forEach((button) => button.addEventListener("click", () => setLibraryOpen(false)));
  ...
  const sessionUsername = document.getElementById("sessionUsernameInput");
  const settingsUsername = document.getElementById("usernameInput");
  sessionUsername.addEventListener("input", () => { settingsUsername.value = sessionUsername.value; });
  settingsUsername.addEventListener("input", () => { sessionUsername.value = settingsUsername.value; });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    closeMenus(); document.getElementById("shortcutsPanel").hidden = true; setLibraryOpen(false); document.getElementById("settingsPanel").classList.add("hidden");
  });
  document.getElementById("settingsPanel").addEventListener("click", (event) => { if (event.target === event.currentTarget) event.currentTarget.classList.add("hidden"); });
  document.getElementById("shortcutsPanel").addEventListener("click", (event) => { if (event.target === event.currentTarget) event.currentTarget.hidden = true; });
  ...
  function syncBuildHealthAvailability() { const invincible = document.getElementById("towerInvincibleInput").checked; document.getElementById("towerHealthInput").disabled = invincible; shellNew.querySelectorAll(".health-pip").forEach((pip) => { pip.disabled = invincible; }); }
  ...
  document.getElementById("towerInvincibleInput").addEventListener("change", syncBuildHealthAvailability);
  syncBuildHealthAvailability();
  ...
  globalThis.CosmowarUIShell = { get current() { return document.documentElement.dataset.uiShell || "new"; }, set: (shell) => applyShell(shell, true), setTheme, closeLibrary: () => setLibraryOpen(false), toggleLibrary: () => document.getElementById("shapeLibraryToggleBtn").click(), openShortcuts: () => { document.getElementById("shortcutsPanel").hidden = false; }, closeOverlays: () => { closeMenus(); document.getElementById("shortcutsPanel").hidden = true; } };
}());
