(function () {
  ...
  function menusMarkup() {
    return `<div class="menu-popover" id="menuFile" role="menu" hidden>${menuItem("new-map", "New map", "", "plus")}${menuItem("", "Import map", "", "upload-simple", "importBtn")}${menuItem("", "Export map", "", "download-simple", "exportBtn")}<hr>${menuItem("", "Export saved shapes", "", "download-simple", "exportCustomShapesBtn")}${menuItem("", "Import saved shapes", "", "upload-simple", "importCustomShapesBtn")}</div>
      ...
      <div class="menu-popover" id="menuView" role="menu" hidden>${menuItem("fit", "Zoom to fit map", "F", "magnifying-glass")}<div class="menu-heading">Grid</div>${menuItem("grid-visible", "Show grid", "G", "grid-four")}${menuItem("grid-major", "Emphasise every 5th line", "", "check-circle", "", "data-grid-dependent")}${menuItem("origin-axes", "Show origin axes", "", "check-circle", "", "data-grid-dependent")}<hr><div class="menu-heading">Snapping</div>${menuItem("grid-snap", "Snap to grid", "", "check-circle")}${menuItem("object-snap", "Snap to objects", "", "check-circle")}${menuItem("move-snap", "Snap while moving", "", "check-circle")}${menuItem("build-snap", "Snap while building", "", "check-circle")}</div>`;
  }
  ...
  function settingsMarkup() {
    return `<div id="settingsPanel" class="modal-scrim hidden"><section class="settings-modal" role="dialog" aria-modal="true" aria-label="Settings"><header>${icon("gear-six")}<h2>Settings</h2><button id="settingsCloseBtn" class="icon-btn" aria-label="Close settings">${icon("x")}</button></header><div class="settings-layout"><nav role="tablist"><button class="settings-tab active" data-settings-tab="canvas">${icon("grid-four", "sm")}Canvas and grid</button><button class="settings-tab" data-settings-tab="snapping">${icon("magnet", "sm")}Snapping</button><button class="settings-tab" data-settings-tab="rules">${icon("shield-check", "sm")}Map rules</button><button class="settings-tab" data-settings-tab="multiplayer">${icon("users-three", "sm")}Multiplayer</button><button class="settings-tab" data-settings-tab="data">${icon("floppy-disk", "sm")}Import and export</button></nav><div class="settings-content">
      <section data-settings-pane="canvas"><h3>Canvas and grid</h3><p>Local workspace appearance.</p>${settingsRow("Theme", "Dark or light interface.", '<select id="themeSelect"><option value="dark">Dark</option><option value="light">Light</option></select>')}${settingsRow("Show grid", "Draw grid and origin axes.", switchControl("gridVisibleInput", "", true))}${settingsRow("Grid size", "Spacing in map units.", '<input id="gridSizeInput" type="number" value="48" min="4" max="1000" step="1">')}${settingsRow("Grid line thickness", "Line width in pixels.", '<input id="gridLineWidthInput" type="number" value="1" min="0.25" max="8" step="0.25">')}${settingsRow("Emphasise every 5th line", "Makes distances easier to count.", switchControl("gridMajorVisibleInput", "", true), "data-grid-dependent")}${settingsRow("Show origin axes", "Draw the 0,0 crosshair.", switchControl("originAxesVisibleInput", "", true), "data-grid-dependent")}</section>
      ...
      <section data-settings-pane="data" hidden><h3>Import and export</h3><p>Map files and saved shapes move independently.</p>${settingsRow("Map files", "Import JSON or TXT and export JSON from the File menu.", '<span class="readonly">File menu</span>')}${settingsRow("Foreign map conversion", "Starts automatically when a TXT file is imported.", '<span class="readonly">Automatic</span>')}${settingsRow("Saved shapes", "Import and export the library from the File menu.", '<span class="readonly">File menu</span>')}</section>
      </div></div></section></div>`;
  }
