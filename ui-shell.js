(function () {
  ...
  function menuItem(action, label, key = "", iconName = "caret-right", id = "") {
    return `<button role="menuitem" ${id ? `id="${id}"` : ""} ${action ? `data-action="${action}"` : ""}>${icon(iconName, "sm")}<span>${label}</span>${key ? `<kbd>${key}</kbd>` : ""}</button>`;
  }
  ...
  shellNew.querySelectorAll(".menu-popover button").forEach((button) => button.addEventListener("click", () => setTimeout(closeMenus)));
  ...
  document.addEventListener("keydown", (event) => {
    ...
    closeMenus(); document.getElementById("shortcutsPanel").hidden = true; document.getElementById("shapeLibrary").hidden = true; document.getElementById("settingsPanel").classList.add("hidden");
  });
